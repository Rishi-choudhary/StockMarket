...
def wallet():
    ...
    for row in rows:
        ...
        # convert price and total to inr format
        row['price'] = inr(row['price'])
        row['total'] = inr(row['total'])

    return render_template("wallet.html", rows=rows, cash=inr(cash), sum=inr(sum))
